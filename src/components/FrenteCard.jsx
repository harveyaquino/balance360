...
function ScoreBar({ score }) {
  const color = score >= 75 ? 'bg-balance360-success'
              : score >= 50 ? 'bg-balance360-accent'
              : score >= 30 ? 'bg-balance360-warn'
...
export function FrenteCard({ name, data }) {
  ...
  // Sanitizar todos los strings del agente antes de renderizar
    ...
    <div className="frente-card animate-fade-in-up">
      <div className="flex items-center justify-between mb-3">
        ...
        <span className="font-mono text-sm font-semibold" style={{
          color: data.score >= 75 ? '#00E676'
               : data.score >= 50 ? '#00E5FF'
               : data.score >= 30 ? '#FFB347'
               : '#FF4757'
        }}>
          {data.score}
        </span>
        ...
        <div className="mt-3">
          ...
          <ul className="space-y-1">
            {data.hallazgos.map((h, i) => (
              <li key={i} className="text-balance360-text text-xs flex gap-2">
        ...
        <div className="mt-3">
          ...
          <ul className="space-y-1">
            {data.oportunidades.map((o, i) => (
              <li key={i} className="text-balance360-success text-xs flex gap-2">
